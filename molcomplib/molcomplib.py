...
class MolCompass:
    def __init__(self, descriptorsType="ecfp_3_2048", modelDescription=None):
        ...
        if not modelType:
            raise NotImplementedError("Other types of models are not implemented yet")
        ...
        fileName = os.path.join(
            ROOT_DIR, "bin", "{}_{}.pkl".format(descriptorsType, "2d")
        )  # Fixme: add model description
    ...
    def _apply_ecfp6_2d(self, input):
        for layer in range(1, 5):
            ...
            if layer != 4:
                input = apply_batchnorm_numpy(
                    input,
                    self.parameters["bn" + str(layer) + ".weight"],
            ...
            if isinstance(data, pd.DataFrame):
                smilesColumn = [
                    ...
                    in [
                        ...
                        "mols",
                        "smi",
                        "smiles_processed_mcv",
                    ]
                ]
                assert (
                    len(smilesColumn) == 1
                ), "Dataframe should contain ONLY one smiles column, but found: {}".format(
                    smilesColumn
                )
